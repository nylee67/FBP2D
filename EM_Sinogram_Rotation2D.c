...
void		Sinogram_Rotation2D(double ***, double ***, Parameter);
void		BackSinogram_Rotation2D(double ***, double ***, Parameter);

void		EM_Sinogram_Rotation2D(
	double			***Sinogram_Y,
	double			***orig_f,
	Parameter		par)
{
		...
		for(ip=0; ip<par.N_Sinogram_Plate; ip++)
			for(ia=0; ia<par.N_Sinogram_Angle; ia++)
				for(iu=0; iu<par.N_Sinogram_Bin; iu++)
					all_1_Y[ip][ia][iu] = 1.0;
					
	//
	//	Ready for Iteration[
	//
		BackSinogram_Rotation2D(all_1_Y, rate_f, par); 
	...
	//
		...
		for(n=0; n<par.N_Iteration; n++){
			...
			Sinogram_Rotation2D(Y, old_f, par);

			for(ip=0; ip<par.N_Sinogram_Plate; ip++)
				for(ia=0; ia<par.N_Sinogram_Angle; ia++)
					for(iu=0; iu<par.N_Sinogram_Bin; iu++)
						if(Y[ip][ia][iu] > EPSILON)
							Y[ip][ia][iu] = Sinogram_Y[ip][ia][iu] / Y[ip][ia][iu];
						else
							Y[ip][ia][iu] = 0.0;
										
			BackSinogram_Rotation2D(Y, new_f, par);
